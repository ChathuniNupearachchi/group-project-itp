...
function EditPayroll() {
    ...
    const handleSubmit = async (e) => {
        ...
        userRequest.put("/payroll/" + id, { orderId, itemCode, single, qty, quality, damaged, deduction, net, additional })
            .then(res => {
    ...
    return (
        <AdminLayout>
                <form className="edit-payroll-form-container" onSubmit={handleSubmit}>
                    <span className="tagline-edit-payroll">EDIT SUPPLIER PAYMENT DETAILS<hr></hr></span>
                    <div className="column-container">
                        <div className="edit-payroll-column">
                            <span className="input-title">Order ID :</span>
                            <input className="input-field" value={orderId} onChange={(e) => setOrderId(e.target.value)} required />
                            <span className="input-title">Item Code :</span>
                            <input className="input-field" value={itemCode} onChange={(e) => setItemCode(e.target.value)} required />
                            <span className="input-title">Single Item Price :</span>
                            <input className="input-field" value={single} onChange={(e) => {
                            ...
                            <input className="input-field" value={qty} onChange={(e) => setQty(e.target.value)} type="number" required />
                            <span className="input-title">Quality Product Amount :</span>
                            <input className="input-field" value={quality} onChange={(e) => {
                                setQuality(e.target.value);
                                calculateNet(single, e.target.value);
                            }} type="number" required />
                            <span className="input-title">Damaged Product Amount :</span>
                            <input className="input-field" value={damaged} onChange={(e) => {
                            ...
                            <input className="input-field" value={additional} onChange={(e) => setAdditional(e.target.value)} />
