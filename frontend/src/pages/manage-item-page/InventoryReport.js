...
import reportHeader from '../../assets/reportHeader.png.jpg';
...
const InventoryReport = ({data}) => {
    ...
    const downloadPDF = () => {
        ...
        doc.text("Inventory Details Report", (pageWidth / 2), 60, { align: "center" });
        // Underline the text
        const textWidth = doc.getStringUnitWidth("Inventory Details Report") * doc.internal.getFontSize() / doc.internal.scaleFactor;
        ...
        doc.autoTable({
          head: [[ "Name", "Category", "Price","Quantity","Added Date","Update Date"]],
          body: data.map((item) => {
            return [item.name,  item.category, item.price, item.quantity,item.createdAt,item.updatedAt];
          }),
